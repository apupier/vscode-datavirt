...
export async function isResourceDeployed(name: string, type: string): Promise<boolean> {
	if (constants.REFERENCE_TYPE_CONFIGMAP === type) {
		return await isConfigMapDeployed(name);
	} else if (constants.REFERENCE_TYPE_SECRET === type) {
		return await isSecretDeployed(name);
	}
	return false;
}

export async function isSecretDeployed(name: string): Promise<boolean> {
	const k8sApi: kubectlapi.API<kubectlapi.KubectlV1> = await kubectlapi.extension.kubectl.v1;
	if (k8sApi && k8sApi.available) {
		try {
			const res: kubectlapi.KubectlV1.ShellResult = await k8sApi.api.invokeCommand(`get secret ${name}`);
			if (res.code === 0) {
				return true;
			}
		} catch (error) {
...
export async function isConfigMapDeployed(name: string): Promise<boolean> {
	const k8sApi: kubectlapi.API<kubectlapi.KubectlV1> = await kubectlapi.extension.kubectl.v1;
	if (k8sApi && k8sApi.available) {
		try {
			const res: kubectlapi.KubectlV1.ShellResult = await k8sApi.api.invokeCommand(`get configmap ${name}`);
			if (res.code === 0) {
				return true;
			}
		} catch (error) {
...
export async function undeploySecret(name: string) {
	const k8sApi: kubectlapi.API<kubectlapi.KubectlV1> = await kubectlapi.extension.kubectl.v1;
	if (k8sApi && k8sApi.available) {
		try {
			const res: kubectlapi.KubectlV1.ShellResult = await k8sApi.api.invokeCommand(`delete secret ${name}`);
			if (res.code !== 0) {
				extension.log(`Unable to delete secret ${name}.`);
			}
		} catch (error) {
			extension.log(error);
		}
...
export async function undeployConfigMap(name: string) {
	const k8sApi: kubectlapi.API<kubectlapi.KubectlV1> = await kubectlapi.extension.kubectl.v1;
	if (k8sApi && k8sApi.available) {
		try {
			const res: kubectlapi.KubectlV1.ShellResult = await k8sApi.api.invokeCommand(`delete configmap ${name}`);
			if (res.code !== 0) {
				extension.log(`Unable to delete configmap ${name}.`);
			}
		} catch (error) {
			extension.log(error);
		}
...
export async function undeployResource(name: string, type: string) {
	if (constants.REFERENCE_TYPE_CONFIGMAP === type) {
		await undeployConfigMap(name);
	} else if (constants.REFERENCE_TYPE_SECRET === type) {
		await undeploySecret(name);
	}
}
