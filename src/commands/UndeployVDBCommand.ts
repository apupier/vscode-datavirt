...
import { getReferenceFilesFromDataSources, getReferenceFilesFromEnvironment } from './DeployVDBCommand';

export async function undeployVDBOnlyCommand(prjNode: DVProjectTreeNode, suppressOutputs: boolean = false) {
	if (await utils.isVDBDeployed(prjNode.dvConfig.metadata.name)) {
		await handleUndeploy(prjNode.file, prjNode.dvConfig.metadata.name, suppressOutputs);
	} else {
		vscode.window.showInformationMessage(`The VDB ${prjNode.file} is not deployed yet. Skipped undeploy action.`);
	}
}

export async function undeployVDBFullCommand(prjNode: DVProjectTreeNode) {
	// determine configmaps / secrets to undeploy first
	await getReferenceFilesFromDataSources(prjNode, true);
	await getReferenceFilesFromEnvironment(prjNode, true);
